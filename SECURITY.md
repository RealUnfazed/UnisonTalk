...
- **If Phasetime SSO is configured** (see README's "Phasetime SSO" section — it's optional), `PHASETIME_CLIENT_SECRET` deserves the same care as `SESSION_SECRET`/`CLOUD_ENCRYPTION_KEY`: it's what lets the server exchange an authorization code for an access token, and it never touches the browser. Treat it as a real credential — don't commit it, don't log it, keep it in a proper secrets manager beyond local development. Accounts created via SSO have no local password at all (by design — see `server/controllers/ssoController.js`), so anyone with access to a Phasetime identity that's linked to a UnisonTalk account can sign in as that account; this is the expected trust relationship of using SSO, not a bug, but it does mean the security of a linked account is only as strong as the Phasetime instance it's linked to.
